...
// This returns a JSON or XML with the Bookss
	...
	public ResponseEntity<List<book>> getBooksByName(@RequestParam String author) {
		return new ResponseEntity<>(BooksRepository.findByauthorIgnoreCase(author), HttpStatus.OK);

	}

	@GetMapping(path = "/search/book")
	public ResponseEntity<List<book>> getBooksBybookname(@RequestParam String bookname) {
		return new ResponseEntity<>(BooksRepository.findBybooknameIgnoreCase(bookname), HttpStatus.OK);

	}

	@GetMapping(path = "/search/top10")
	public ResponseEntity<List<book>> findBycopiessold(Integer copiessold) {
		return new ResponseEntity<>(BooksRepository.findBycopiessold(copiessold), HttpStatus.OK);
